...
test("usage service opencode happy path returns plan and three quota windows", async () => {
  const resetAt = "2026-09-30T07:08:09.000Z";
  globalThis.fetch = async () =>
    new Response(
      JSON.stringify({
        usage: {
          rolling: { status: "ok", percent: 25, resetsAt: resetAt },
          weekly: { status: "ok", percent: 50, resetsAt: resetAt },
          monthly: { status: "ok", percent: 10, resetsAt: resetAt },
        },
      }),
      { status: 200, headers: { "content-type": "application/json" } }
    );
  ...
  const result: any = await usageService.getUsageForProvider({
    id: `usage-hardening-opencode-${Date.now()}`,
    provider: "opencode",
    apiKey: "oc-happy-key",
  });
  ...
  assert.deepEqual(
    ["session", "weekly", "mcp_monthly"].map((key) => result.quotas[key].total),
    [12, 30, 60]
  );
});
