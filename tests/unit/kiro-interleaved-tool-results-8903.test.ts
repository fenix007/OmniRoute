...
test("kiro #8903: mixed Anthropic tool_result and text keeps the batch grouped", () => {
  const payload = build([
    { role: "user", content: "check two things" },
    PARALLEL_TOOL_CALLS,
    { role: "tool", tool_call_id: "call_A", content: "res A" },
    { role: "assistant", content: "DEFERRED_BEFORE_MIXED_RESULT" },
    {
      role: "user",
      content: [
        { type: "tool_result", tool_use_id: "call_B", content: "res B" },
        { type: "text", text: "USER_TEXT_AFTER_GROUPED_RESULTS" },
      ],
    },
  ...
  const turns = collectTurns(payload);
  const batchIdx = turns.findIndex(
    (turn) => JSON.stringify((turn.toolResults ?? []).toSorted()) === '["call_A","call_B"]'
  );
  const assistantIdx = turns.findIndex((turn) =>
    turn.content.includes("DEFERRED_BEFORE_MIXED_RESULT")
  );
  const userTextIdx = turns.findIndex((turn) =>
    turn.content.includes("USER_TEXT_AFTER_GROUPED_RESULTS")
  );

  assert.ok(batchIdx >= 0, "both tool results must remain in one Kiro user turn");
  assert.ok(assistantIdx > batchIdx, "interleaved assistant prose follows the grouped results");
  assert.ok(userTextIdx > assistantIdx, "the mixed message's user text keeps its original order");
  assert.match(turns[userTextIdx].content, /next question/);
});
...
test("kiro #8903: interleaved tool batch preserves the fork JSON response contract", () => {
  const payload = buildKiroPayload(
    "claude-sonnet-4.5",
    {
      ...
      messages: [
        ...
        PARALLEL_TOOL_CALLS,
        { role: "tool", tool_call_id: "call_A", content: "res A" },
        { role: "assistant", content: "DEFERRED_JSON_TEXT" },
        { role: "tool", tool_call_id: "call_B", content: "res B" },
        { role: "user", content: "return the verdict" },
      ],
      response_format: {
        type: "json_schema",
        json_schema: {
  ...
  const turns = collectTurns(payload);
  const batchTurn = turns.find((turn) => (turn.toolResults ?? []).length > 0);
  assert.ok(batchTurn, "the grouped tool-result turn must survive");
  assert.deepEqual([...batchTurn.toolResults].sort(), ["call_A", "call_B"]);

  const deferredIdx = turns.findIndex((turn) => turn.content.includes("DEFERRED_JSON_TEXT"));
  const currentIdx = turns.findIndex((turn) => turn.current);
  assert.ok(deferredIdx > turns.indexOf(batchTurn), "deferred prose follows the grouped results");
  assert.ok(currentIdx > deferredIdx, "the JSON-constrained user turn remains current");

  const currentContent = turns[currentIdx].content;
  assert.match(currentContent, /return the verdict/);
  assert.match(currentContent, /<response_format>json<\/response_format>/);
  assert.match(currentContent, /You must respond with valid JSON that strictly follows/);
  assert.match(currentContent, /no markdown code fences/);
  assert.ok(currentContent.trimEnd().endsWith("</system-reminder>"));
});
